...
Open the browser-based SSH console for your instance and run:

```bash
curl -fsSL https://raw.githubusercontent.com/cto-agent/cto-agent/main/install.sh | sudo bash
```

That single command will:

1. Install Go (if not already present or if version is outdated)
...
The database is **never deleted** during installs or updates — your config, conversation history, and memories are preserved every time.
...
SSH into the instance and run the exact same install command again:
