...
// cfGraphQLResponse is the top-level GraphQL response envelope.
type cfGraphQLResponse struct {
        Data   cfGraphQLData    `json:"data"`
        Errors []cfGraphQLError `json:"errors"`
}

type cfGraphQLError struct {
        Message string `json:"message"`
}

type cfGraphQLData struct {
        Viewer struct {
                Zones []struct {
                        HttpRequests1hGroups []struct {
                                Sum cfAnalyticsSums `json:"sum"`
                        } `json:"httpRequests1hGroups"`
                } `json:"zones"`
        } `json:"viewer"`
}

type cfAnalyticsSums struct {
...
// cfGetAnalytics returns basic zone analytics using the Cloudflare GraphQL Analytics API.
  ...
  viewer {
    zones(filter: {zoneTag: %q}) {
      httpRequests1hGroups(
        limit: 10000,
        filter: {datetime_geq: %q, datetime_leq: %q}
      ) {
        sum {
          requests
          bytes
        ...
        req, err := http.NewRequestWithContext(ctx, "POST", "https://api.cloudflare.com/client/v4/graphql", bytes.NewReader(raw))
        if err != nil {
                return "ERROR: " + err.Error()
        }
        req.Header.Set("Authorization", "Bearer "+token)
        ...
        if resp.StatusCode >= 300 {
                return fmt.Sprintf("ERROR: Cloudflare GraphQL returned %d: %s", resp.StatusCode, truncate(string(data), 400))
        }

        var res cfGraphQLResponse
        if err := json.Unmarshal(data, &res); err != nil {
                return "ERROR: parse: " + err.Error()
        }
        if len(res.Errors) > 0 {
                ...
                return fmt.Sprintf("ERROR: %s", strings.Join(msgs, "; "))
        }
        zones := res.Data.Viewer.Zones
        if len(zones) == 0 || len(zones[0].HttpRequests1hGroups) == 0 {
                return fmt.Sprintf("last %dh — no data available", sinceHours)
        }

        var totals cfAnalyticsSums
        for _, g := range zones[0].HttpRequests1hGroups {
        ...
        mb := float64(totals.Bytes) / 1048576.0
        return fmt.Sprintf(
