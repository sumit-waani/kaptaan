...
type Agent interface {
        ...
        // GetStatus returns the live agent state (from KV) and recalculated trust score.
...
func (b *Bot) handleStatus(ctx context.Context) {
        ...
        // Read live state from KV and recalculate trust score
        state, trust := b.agent.GetStatus(ctx)
...
Trust:   %.1f%%
Plan:    %s`,
                proj.Name, state, trust, planInfo))
}
