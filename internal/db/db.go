...
// migrate creates the new schema and drops every legacy table that the old
// architecture used. Running this on an existing DB is destructive but the
// data was Kaptaan's internal state, not user-authored content (the source of
// truth is GitHub + plan files on disk).
func (d *DB) migrate(ctx context.Context) error {
	// Drop legacy tables first so leftover FK-bound rows don't block schema
	// changes. Order doesn't matter because of CASCADE.
	legacy := []string{
	...
	// Older project_docs/llm_usage might already exist with a different shape;
	// drop and re-create to guarantee the new schema.
	for _, t := range []string{"project_docs", "memories", "llm_usage"} {
		if _, err := d.pool.Exec(ctx, "DROP TABLE IF EXISTS "+t+" CASCADE"); err != nil {
			return fmt.Errorf("drop %s: %w", t, err)
		}
	}
	// Trim legacy columns from project (status / trust_score) — best-effort.
