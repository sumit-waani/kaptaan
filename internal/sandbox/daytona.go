...
// lifecycle and the in-sandbox Toolbox HTTP API for shell + filesystem ops.
//
...
func Create(ctx context.Context, apiKey, orgID string, _ int) (*Sandbox, error) {
	...
	sb := &Sandbox{ID: ws.ID, apiKey: apiKey, orgID: orgID}
...
// NewHandle returns a Sandbox handle from a stored sandbox ID without any
...
// Connect starts a stopped (auto-paused) Daytona sandbox by ID and waits
// until the toolbox is reachable. id is the plain sandbox ID as stored in DB.
func Connect(ctx context.Context, apiKey, orgID, id string) (*Sandbox, error) {
	...
	// POST /sandbox/{id}/start is idempotent — safe to call even if running.
...
// Ping returns true if the sandbox is currently in "started" state.
func (s *Sandbox) Ping(ctx context.Context) bool {
	hctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
	res, err := doReq(hctx, s.apiKey, s.orgID, http.MethodGet, "/sandbox/"+s.ID, nil)
	if err != nil {
		return false
	}
	defer res.Body.Close()
...
func (s *Sandbox) Run(ctx context.Context, cmd, cwd string, env map[string]string, timeout time.Duration) (*RunResult, error) {
	...
	res, err := doReq(rctx, s.apiKey, s.orgID, http.MethodPost,
		"/sandbox/"+s.ID+"/toolbox/process/execute", payload)
	if err != nil {
		return nil, err
	}
	defer res.Body.Close()
...
func (s *Sandbox) WriteFile(ctx context.Context, path string, content []byte) error {
	...
	u := apiBase + "/sandbox/" + s.ID + "/toolbox/files/upload?path=" + url.QueryEscape(path)
	...
	if s.orgID != "" {
		req.Header.Set("X-Daytona-Organization-ID", s.orgID)
	}
...
func (s *Sandbox) ReadFile(ctx context.Context, path string) ([]byte, error) {
	u := apiBase + "/sandbox/" + s.ID + "/toolbox/files/download?path=" + url.QueryEscape(path)
